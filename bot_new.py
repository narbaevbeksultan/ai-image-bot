...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text(
        ...
        "• 🎨 Советы и помощь\n\n"
        "💰 **Платные функции:**\n"
        "• 🎬 Создать видео (требует кредиты Replicate)\n"
        "• 🪙 Пакеты кредитов для генераций",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def handle_credit_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        payment_url = betatransfer_api.get_payment_url(payment_result['payment_id'])
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("✅ Проверить статус", callback_data=f"check_payment:{payment_result['payment_id']}")],
